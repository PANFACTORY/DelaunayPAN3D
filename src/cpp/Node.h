...
public:
	...
	Node<T> Node<T>::operator+(const Node<T>& _node) {
		return Node<T>(this->x + _node.x, this->y + _node.y, this->z + _node.z, -1, -1);
	}
	...
	template<class T>
	Node<T> Node<T>::operator*(const Node<T>& _node) {
		return Node<T>(this->y*_node.z - this->z*_node.y, this->z*_node.x - this->x*_node.z, this->x*_node.y - this->y*_node.x, -1, -1);
	}
